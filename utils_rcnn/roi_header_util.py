...
import utils_rcnn.boxes_utils as box_op
from utils_rcnn.det_utils import *
...
class RoIHeads(torch.nn.Module):
    ...
    def postprocess_detections(self,
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            ...
            boxes_befor_nms, scores, labels = boxes[keep], scores[keep], labels[keep]

            # non-maximun suppression, independently done per class
            # nms를 적용함
            print(boxes_befor_nms.shape)
            bf_nms_boxes.append(boxes_befor_nms)
            # 执行nms处理，执行后的结果会按照scores从大到小进行排序返回
            keep = box_op.batched_nms(boxes_befor_nms, scores, labels, self.nms_thresh)

            # keep only topk scoring predictions
            # 获取scores排在前topk个预测目标
            keep = keep[:self.detection_per_img]
            boxes_after_nms, scores, labels = boxes_befor_nms[keep], scores[keep], labels[keep]
            print(boxes_after_nms.shape)
            af_nms_boxes.append(boxes_after_nms)
            
            all_boxes.append(boxes_after_nms)
            all_scores.append(scores)
            all_labels.append(labels)

        return all_boxes, all_scores, all_labels, bf_nms_boxes
    ...
    def forward(self,
        ...
        if self.training:
            ...
            # 코드 변경
            boxes, scores, labels, self.bf_nms_boxes = self.postprocess_detections(class_logits, box_regression, proposals, image_shapes)
        ...
        return result, losses, self.bf_nms_boxes
