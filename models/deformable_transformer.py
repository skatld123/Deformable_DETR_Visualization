...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
                 ...
                 two_stage=False, two_stage_num_proposals=300, faster_rcnn=True):
        super().__init__()
    ...
    ##TODO 같은 메서드처럼 Faster R-CNN의 입력을 받을 수 있도록 하기 
    ...
    def gen_encoder_output_proposals_2(self, memory, memory_padding_mask, spatial_shapes, frcnn_boxes):
        ...
        # print(proposals[2])
        # faster rcnn 박스를 받아서 spatial shape에 맞게 각 좌표를 수정
        # 그리고 concat
        # _cur = 0
        # for lvl, (H_, W_) in enumerate(spatial_shapes):
            # for boxes_tensor in frcnn_boxes :
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, frcnn_boxes=None):
        ...
        bs, _, c = memory.shape
        if self.two_stage and (self.faster_rcnn == True):
            # 인코더의 출력과 영역제안
            print("mask_flatten")
            ...
            # output_proposals : torch.Size([1, 14512, 4])
            # output_memory : torch.Size([1, 14512, 256])
            ...
            # 여기서(위) Faster-RCNN의 자동차영역을 가져오는 방법을 실행..?
        ...
        # elif self.two_stage and self.faster_rcnn:
        #     print("RPN을 사용하기") 
        #     # 인코더의 출력과 영역제안
        #     print("mask_flatten")
        #     print(mask_flatten.shape)
        ...
        #     print(memory.shape)
        #     a = frcnn_boxes
        #     # @@TODO 생각해보니 여기에서 이미지는 피처맵이다.
        #     output_memory, output_proposals = self.gen_encoder_output_proposals(memory, mask_flatten, spatial_shapes, frcnn_boxes)
        ...
        # decoder
        # 여기서 object query가 뭘까?
        hs, inter_references = self.decoder(tgt, reference_points, memory,
                                            spatial_shapes, level_start_index, valid_ratios, query_embed, mask_flatten)
        # output = hs.transpose(1, 2), memory.permute(1, 2, 0).view(bs, c, h, w)
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def visualize_sampling_points(reference_points, sampling_locations) :
        ...
        for l in range(4):  # level의 수에 따라 조절해야 합니다.
            ...
            plt.title(f'Layer_{_}/lvl_{l}')
            plt.savefig(f"/root/Deformable-DETR/result_sampling/Layer_{_}_lvl_{l}.jpg")
            plt.close()
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        ...
        for _, layer in enumerate(self.layers):
            ...
            # 샘플링 포인트 시각화
            # self.visualize_sampling_points(reference_points, sampling_locations)
        return output
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            # print(f'output = {output.shape}, query_pos={query_pos.shape}, reference_points_input={reference_points_input.shape}, src={src.shape}')
            # print(f'src_spatial_shapes={src_spatial_shapes.shape}, src_level_start_index={src_level_start_index.shape}, src_padding_mask={src_padding_mask.shape}')
