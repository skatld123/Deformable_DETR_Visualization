...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, frcnn_boxes=None):
        ...
        # 만약 기존 3개의 피처맵보다 많은 수의 lvl을 하이퍼 파라미터로 설정하였을 경우, 
        # 아래 과정을 통해 feature map을 하나 더 만든다.
        ...
        if frcnn_boxes != None : 
            ...
            scaled_boxes = []
            for idx, box in enumerate(frcnn_boxes) :
                bx, by, bw, bh = box[0], box[1], box[2], box[3]
                bx = torch.tensor([bx / h])
                by = torch.tensor([by / w])
                bw = torch.tensor([bw / w])
                bh = torch.tensor([bh / h])
        ...
        outputs_classes = []
        outputs_coords = []
        # 디코더 레이어 수만큼 반복하여 cls, box 임베딩을 거침
        ...
        # 디코더의 마지막 레이어의  [1,300,4]를 가져와서 사용
        ...
        return out, topk_coords_unact, outputs_coord
